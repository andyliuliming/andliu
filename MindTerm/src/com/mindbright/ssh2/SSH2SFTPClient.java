...
public final class SSH2SFTPClient extends SSH2SFTP
    ...
    private class ReplyLock {
        ...
        protected synchronized SFTPPacket expect(int timeout)
        throws SFTPException {
	    timeout *= 1000;
	    long start = System.currentTimeMillis();
            while(replyPkt == null) {
		if (System.currentTimeMillis() - start > timeout)
		    throw new SFTPException("SFTP timeout");
                try {
                    this.wait(timeout);
	...
	this.timeout = this.connection.getPreferences().getIntPreference(SSH2Preferences.SFTP_TIMEOUT);
        ...
        pkt.reset();
        pkt.failsafeReadFrom(in, this.timeout);
	...
	handle.asyncWait(0, this.timeout);
    }

    private void asyncWait(FileHandle handle, int n) throws SFTPException {
        handle.asyncWait(n, this.timeout);
    }
        ...
        try {
            while (len == -1 || foffs < len) {
                ...
                if (useAsyncRead && ++cnt == MAX_OUTSTANDING_ASYNCH) {
                    cnt = 0;
                    asyncWait(handle, MAX_OUTSTANDING_ASYNCH/2);
                }
            }
    ...
    throws SFTPException {
        ...
        replyLocks.put(id, reply);
        transmit(pkt);
        return reply.expect(this.timeout);
    }
