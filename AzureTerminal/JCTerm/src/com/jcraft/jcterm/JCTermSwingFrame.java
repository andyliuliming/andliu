...
public class JCTermSwingFrame extends JFrame implements Frame, ActionListener,
		Runnable {
	...
	public void run() {
		...
		while (thread != null) {
			try {
				int port = 22;
				try {
					String[] destinations = JCTermSwing.getCR()
							.load(configName).destinations;
					...
					if (_host.indexOf(':') != -1) {
						try {
							port = Integer.parseInt(_host.substring(_host
									.indexOf(':') + 1));
						} catch (Exception eee) {
						}
				...
				try {
					...
					jschsession = JSchSession.getSession(user, null, host,
							port, ui, proxy);
	...
	public class MyUserInfo implements UserInfo, UIKeyboardInteractive {
		public boolean promptYesNo(String str) {
			...
			int foo = JOptionPane.showOptionDialog(JCTermSwingFrame.this.term,
					str, "Warning", JOptionPane.DEFAULT_OPTION,
	...
	public void setCompression(int compression) {
		...
		if (jschsession != null) {
			if (compression == 0) {
				...
				jschsession.getSession().setConfig("compression.s2c",
						"zlib@openssh.com,zlib,none");
				jschsession.getSession().setConfig("compression.c2s",
						"zlib@openssh.com,zlib,none");
	...
	private void _setFontSize(int size) {
		...
		frame.setSize(mwidth + term.getTermWidth(),
				mheight + term.getTermHeight());
		term.clear();
		term.redraw(0, 0, term.getWidth(), term.getHeight());
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (action.equals("Open SHELL Session...")
			...
			String display = JOptionPane.showInputDialog(this,
					"XDisplay name (hostname:0)", (xhost == null) ? "" : (xhost
							+ ":" + xport));
			try {
				if (display != null) {
			...
			try {
				...
				if (foo == null)
					return;
				int port1 = Integer
						.parseInt(foo.substring(0, foo.indexOf(':')));
				foo = foo.substring(foo.indexOf(':') + 1);
				String host = foo.substring(0, foo.indexOf(':'));
				int port2 = Integer
						.parseInt(foo.substring(foo.indexOf(':') + 1));
	...
	private String promptDestination(JComponent term, String[] destinations) {
		...
		JOptionPane pane = new JOptionPane(jb, JOptionPane.QUESTION_MESSAGE,
