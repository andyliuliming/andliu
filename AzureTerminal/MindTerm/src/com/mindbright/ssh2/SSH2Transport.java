 ...
 * <code>SSH2TransportEventHandler</code> which are provided to the
 * constructor.
 * <p>
...
public class SSH2Transport implements NIOCallback, NQueueCallback {
    ...
    /**
     * Context for transport transceiver/receiver. Holds the encryption
    ...
    private AtomicBoolean disconnectInProgress = new AtomicBoolean(false);
     ...
     * immediately).
     *
     * @return a boolean indicating if key exchange was successful or
     *         not.
     *
    ...
    private synchronized void changeKeys(boolean transmitter)
        ...
        try {
            ...
            TransceiverContext ctx =
                SSH2TransportPDU.createTransceiverContext
                (cipherName, cDiscard, macName, mLen, sshCompName, tpIn);
    ...
    private volatile static int cache_total_used = 0;
    private volatile static int cache_total_max = 0;

    private static int getAllowedCacheSize (int n) {
	if (cache_total_used + n < cache_total_max) {
	    cache_total_used += n;
	    return n;
	} else {
	    cache_total_used += 2;
	...
	if (cache_total_max == 0)
	    cache_total_max = this.ourPrefs.getIntPreference(SSH2Preferences.TRANSPORT_CACHE_SIZE_TOTAL);
	cachesize = getAllowedCacheSize(this.ourPrefs.getIntPreference(SSH2Preferences.TRANSPORT_CACHE_SIZE));
        cache_buf = new byte[cachesize * SSH2TransportPDU.PACKET_MAX_SIZE];
            ...
            for (;;) {
                ...
                if (len < need) {
                    if ((buf.limit() - buf.position()) < SSH2TransportPDU.PACKET_MAX_SIZE) {
                        System.arraycopy(cbuf, cachepos, cbuf, 0, len);
                        buf.position(len);
                        cachepos = 0;
                    } else if (len == 0) {
                        buf.clear();
                ...
                pduIn.fillData(cbuf, cachepos, need);
                cachepos += need;
                handleReadData();
     ...
     * This will negotiate the version with our peer.
     */
