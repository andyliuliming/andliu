...
public class SSH2SFTP {
    ...
    public static final class FileHandle {
         ...
         */
        protected synchronized boolean asyncWait(int reqLeft, int timeout)
	 ...
	 * @param timeout Timeout
         */
        public void failsafeReadFrom(NonBlockingInput in, int timeout) throws SFTPException{
            lookForPacket = true;
            internalRead(in, 2);
            waitForPacket(timeout);
        }

        public void readFrom(NonBlockingInput in, int timeout) throws SFTPException {
            internalRead(in, 5);
            waitForPacket(timeout);
        }
        ...
        private void waitForPacket(int timeout) throws SFTPException {
	    timeout *= 1000;
            synchronized(this) {
		long start = System.currentTimeMillis();
                while (!packetReady) {
