...
public class MindTerm extends JApplet implements Runnable, WindowListener {
    ...
    private void applyFocusAndTabKeyWorkaround() {
        try {
            ...
            try {
                java.awt.KeyEventDispatcher ked = (java.awt.KeyEventDispatcher)topParent;
    ...
    public void getAppletParams() {
        ...
        try {
			separateFrame = Boolean.valueOf(getParameter("sepframe"));
		} catch (Exception e) {
            separateFrame = true;
        }
        try {
            SSH.DEBUG = Boolean.valueOf(getParameter("verbose"));
        } catch (Exception e) {
            SSH.DEBUG = false;
        }

        try {
            SSH.DEBUGMORE = Boolean.valueOf(getParameter("debug"));
            SSH.DEBUG = SSH.DEBUGMORE;
        } catch (Exception e) {}

        try {
            quiet = Boolean.valueOf(getParameter("quiet"));
        } catch (Exception e) {
            quiet = true;
        }

        try {
            exitOnLogout = Boolean.valueOf(getParameter("exit-on-logout"));
        } catch (Exception e) {
            exitOnLogout = false;
        }

        try {
            savePasswords = Boolean.valueOf(getParameter("savepasswords"));
        } catch (Exception e) {
            savePasswords = false;
        }

        try {
            startSFTP = Boolean.valueOf(getParameter("startsftp"));
        } catch (Exception e) {}
    ...
    void printHelp() {
        ...
        System.out.println("  --S            Start SFTP module upon connecting.");
    ...
    public void doExit() {
        ...
        if(weAreAnApplet) {
            com.mindbright.nio.Switchboard.stop();
        } else {
            System.exit(0);
        }
    }
