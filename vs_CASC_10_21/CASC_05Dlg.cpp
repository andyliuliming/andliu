...
void CCASC_05Dlg::Msg_WR()
{
	...
	case AUX_CONST_ROT:{
		if(global_aux_hot_status==1)strcpy(s_buf,"500000FF03FF000019000A14010001M*00007000011");
		else strcpy(s_buf,"500000FF03FF000019000A14010001M*00007000010");
		break;}//辅助功能——恒热旋转
	case AUX_CLEAR1:{
	...
	case AUX_ALIGN_ENTER:{
		...
		break;}//辅助功能——对刀进入
	case AUX_HOME_START:{
		if(global_home_start==1)strcpy(s_buf,"500000FF03FF000019000A14010001M*00001300011");
		else strcpy(s_buf,"500000FF03FF000019000A14010001M*00001300010");
		break;}//辅助功能——回原点开始
	case AUX_GAS_OPEN:{
		if(global_home_gas==1)strcpy(s_buf,"500000FF03FF000019000A14010001M*00012800011");
	...
	if(global_opr_wr_rd==RECIEPE_SAVE)
	{
		...
		for(int k=0;k<=2;k++)///////////////////////////////////////////
		{
			if(k==0){global_recipe_pin_pos[0][0]=global_pin_latch;
